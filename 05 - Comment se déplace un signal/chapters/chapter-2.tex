...
\begin{frame}{Scénario}
    ...
    \begin{maketikzfigure}
        ...
        \only<1-2>{
        \draw [thick]
            (0,0) to [american voltage source, l_=$V$, invert] (0,5)
            to [empty led] (2, 5) to [short] (3, 5);
        }
        \only<3->{
        \draw [thick]
            (0,0) to [american voltage source, l_=$V$, invert] (0,5)
            to [full led, color=accent] (2, 5) to [short] (3, 5);
        }
        ...
        \node at (5, 4.5) {1 année lumière};

        \draw [<->] (-1, 0) to (-1, 5);
        \node at (-1.5, 2.5) {$\SI{1}{\meter}$};
    \end{maketikzfigure}
    \end{twocolumns}
